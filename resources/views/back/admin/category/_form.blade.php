...
<div class="mb-3">
    ...
    <select name="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
        ...
        @foreach ($parents as $p)
            <option value="{{ $p->id }}" @selected(old('parent_id', $isEdit ? $category->parent_id : '') == $p->id)>
                {{ $p->name }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3">
    ...
    <select name="status" class="form-control @error('status') is-invalid @enderror">
        <option value="1" {{ old('status', $isEdit ? $category->status : 1) == 1 ? 'selected' : '' }}>Active
        </option>
        <option value="0" {{ old('status', $isEdit ? $category->status : 1) == 0 ? 'selected' : '' }}>Nonaktif
        </option>
    </select>
    @error('status')
        <div class="invalid-feedback">{{ $message }}</div>
